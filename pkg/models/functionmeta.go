...
func (m *FunctionMeta) GetMinReplicaCount() int {
	return fc.GetMinReplicaCount(*m.Labels)
}

// GetMaxReplicaCount gets the maximum replica count for this function
func (m *FunctionMeta) GetMaxReplicaCount() int {
	return fc.GetMaxReplicaCount(*m.Labels)
}

func (m *FunctionMeta) scaleUpRPC(node *Node, c chan *fcdaemon.CreateReply) {
	req, err := fc.PrepareReplicaRequest(*m.Labels, m.EnvVars, m.Image)
	if err != nil {
...
func (m *FunctionMeta) ScaleUp(node *Node, n uint32, rc chan *Replica) {
	c := make(chan *fcdaemon.CreateReply)
