...
func ChangeScheduler(s dataflow.Scheduler) {
	s.CopyResources(scheduler)
	scheduler = s
}
...
func LaunchBackend(s *resmngr.ResourceManagerStates) {
	log.Info().Str("faas-memory version", version.BuildVersion()).
		Str("Last commit message", version.GitCommitMessage).
		Str("commit SHA", version.GitCommitSHA).Msg("")
...
//RegisterPprof registers pprof related handle
func RegisterPprof() {
