...
func (t *TaskState) executeMessage(inputMsg InterStateMessage) {
	...
	if t.balance {
		reb_start := time.Now()
		rebalancedInputs := t.rebalance(bodyJSON)
		reb_dur := time.Since(reb_start)
		log.Info().Dur("rebalance", reb_dur).Msg("")
		if len(rebalancedInputs) == 1 {
			t.executeWithInput(inputJSON, rebalancedInputs[0], &inputMsg, path)
		} else {
			inputLen := len(rebalancedInputs)
			...
			filelog.FileLogger.Info().Str("name", t.BaseState.Name).
				Str("cat", "PERF").Str("ph", "B").Str("pid", t.GetBaseState().DataFlowId).
				Msg("")
			for idx, input := range rebalancedInputs {
				if idx == 0 {
					go func(inputJSON *gabs.Container, bodyJSON *gabs.Container,
						inputMsg *InterStateMessage, path string) {
						defer wg.Done()
						t.Nested = true
						t.executeWithInput(inputJSON, bodyJSON, inputMsg, path)
					}(inputJSON, input, &inputMsg, path)
				} else {
					hintCpy := SchedulerHint{
						Reads:           t.SchedulingHints.Reads,
						Uploads:         t.SchedulingHints.Uploads,
						ScheduledNodeIP: t.SchedulingHints.ScheduledNodeIP,
						FunctionID:      t.SchedulingHints.FunctionID,
					}
					go func(idx int, input *gabs.Container, hint *SchedulerHint) {
						defer wg.Done()
						output, _ := t.sch.RunSingleFunction(t.GetFunctionName(),
							input.String(), hint, t.GetBaseState().DataFlowId, true)
						gather <- Output{
							idx:    idx,
							output: output,
						}
			...
			if inputLen > 1 {
				...
				finalBody, _ := gabs.ParseJSON([]byte(inputMsg.Body))
				for _, output := range outputs {
					out := output.output.(InterStateMessage)
					output, _ := gabs.ParseJSON([]byte(out.Body))
					finalBody.Merge(output)
					inputMsg.Hints.Reads += out.Hints.Reads
					inputMsg.Hints.Uploads += out.Hints.Uploads
					// TODO: change ScheduledNodeIP to a set instead of a string
				}
				inputMsg.Body = finalBody.String()
			}
			filelog.FileLogger.Info().Str("name", t.BaseState.Name).
				Str("cat", "PERF").Str("ph", "E").Str("pid", t.BaseState.DataFlowId).
				Msg("")
